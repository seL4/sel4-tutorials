...
void echo_server(seL4_CPtr ep, seL4_CPtr reply)
{
    ...
    info = seL4_NBSendRecv(ep, info, ep, NULL, reply);
...
int main(void) {
    ...
    * scheduling context to be configured for that node. */
    ...
    sched_control = simple_get_sched_ctrl(&simple, info->nodeID);
    ...
    error = vka_alloc_sched_context_size(&vka, &sc_object, seL4_MinSchedContextBits);
    ...
    error = seL4_SchedContext_Unbind(sc_object.cptr);
    ...
    error = seL4_SchedControl_Configure(sched_control, sc_object.cptr, 1 * US_IN_S, 2 * US_IN_S, 0);
   ...
   /* TASK 6: rebind the scheduling context to the thread we created */
    ...
    error = seL4_SchedContext_Bind(sc_object.cptr, thread.tcb.cptr);
    ...
    error = seL4_SchedControl_Configure(sched_control, sc_object.cptr, 1 * US_IN_S, 2 * US_IN_S, 2);
    ...
    error = vka_alloc_reply(&vka, &reply);
