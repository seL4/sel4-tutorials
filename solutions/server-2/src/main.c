...
int main(void) {
    ...
    error = vka_alloc_sched_context(&vka, &sched_context1);
    ZF_LOGF_IFERR(error, "Failed to allocate first scheduling context object\n");
    error = vka_alloc_sched_context(&vka, &sched_context2);
    ZF_LOGF_IFERR(error, "Failed to allocate second scheduling context object\n");

    /* TODO 2: initialise the scheduling contexts */
    /* hint 1: seL4_SchedControl_Configure */
    /* hint 2: simple_get_sched_ctrl */
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&simple), sched_context1.cptr, THREAD_1_BUDGET, THREAD_2_PERIOD, 0);
    ZF_LOGF_IFERR(error, "Failed to configure scheduling context 1\n");
    error = seL4_SchedControl_Configure(simple_get_sched_ctrl(&simple), sched_context2.cptr, THREAD_2_BUDGET, THREAD_2_PERIOD, 0);
    ZF_LOGF_IFERR(error, "Failed to configure scheduling context 2\n");

    /* TODO 3: initialise the new TCBs */
    /* hint 1: seL4_TCB_Configure() */
    error = seL4_TCB_Configure(tcb_object1.cptr, seL4_CapNull, seL4_CapNull, seL4_Prio_new(THREAD_1_PRIO, THREAD_1_PRIO, 0, 0),
                               sched_context1.cptr, cspace_cap, seL4_NilData, pd_cap, seL4_NilData, ipc_buffer_vaddr1, ipc_frame_object1.cptr);
    error |= seL4_TCB_Configure(tcb_object2.cptr, seL4_CapNull, seL4_CapNull, seL4_Prio_new(THREAD_2_PRIO, THREAD_2_PRIO, 0, 0),
                                sched_context2.cptr, cspace_cap, seL4_NilData, pd_cap, seL4_NilData, ipc_buffer_vaddr2, ipc_frame_object2.cptr);
