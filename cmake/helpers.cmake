...
function(Copy src dest)
  EnsureDir("${dest}")
  execute_process(COMMAND ${CMAKE_COMMAND} -E copy "${src}" "${dest}" RESULT_VARIABLE exit_status)
...
function(DiffFiles res one two)
  execute_process(COMMAND diff -q "${one}" "${two}" RESULT_VARIABLE exit_status
...
function(CopyIfUpdated input output old)
  if(EXISTS ${output})
    ...
    if(template_updated AND instance_updated)
      message(
        FATAL_ERROR "Template has been updated and the instantiated tutorial has been updated. \
...
function(ExecuteGenerationProcess input_dir output_dir generated_files)
  ...
  execute_process(COMMAND ${CMAKE_COMMAND} -E env ${TUTE_COMMAND} OUTPUT_VARIABLE OUTPUT
                  ERROR_VARIABLE OUTPUT RESULT_VARIABLE res)
  if(res)
...
function(GenerateTutorial full_path)
  ...
  if(NOT EXISTS ${base_dir}/${dir}/CMakeLists.txt)
    message(FATAL_ERROR "Could not find: ${base_dir}/${dir}/CMakeLists.txt"
...
function(cdl_ld outfile output_target)
  ...
  add_custom_command(
    OUTPUT "${outfile}"
    COMMAND
      ${capdl_linker_tool} --arch=${KernelSel4Arch} --object-sizes
      $<TARGET_PROPERTY:object_sizes,FILE_PATH> gen_cdl --manifest-in ${CDL_LD_MANIFESTS} --elffile
      ${CDL_LD_ELF} --keys ${CDL_LD_KEYS} --outfile ${outfile}
...
function(cdl_pp manifest_in target)
  ...
  add_custom_command(
    ...
    COMMAND
      ${capdl_linker_tool} --arch=${KernelSel4Arch} --object-sizes
      $<TARGET_PROPERTY:object_sizes,FILE_PATH> build_cnode --manifest-in=${manifest_in} --elffile
      ${CDL_PP_ELF} --ccspace ${CDL_PP_CFILE}
